...
class Seller:
    ...
    def get_seller_delay_wait_time(self):
        ...
        def delay_to_logistic_partner(df):
            days = np.mean(
                (df.shipping_limit_date - df.order_delivered_carrier_date)
                / np.timedelta64(24, 'h'))
            if days < 0:
                return abs(days)
            return 0
    ...
    def get_review_score(self):
        ...
        reviews_df = reviews_df.groupby(
            'seller_id', as_index=False).agg({'dim_is_one_star': 'mean',
                                              ...
                                              'review_score': 'mean'})
        # Rename columns
        reviews_df.columns = ['seller_id', 'share_one_stars',
    ...
    def get_quantity(self):
        ...
        quantity = matching_table.groupby('seller_id', as_index=False)\
            .agg({'order_id': 'count'})
    ...
    def get_ceo_request_data(self):
        
        sellers = self.get_training_data()

        # Get revenue per seller

        sellers['olist_age'] = round((sellers['date_last_sale'] - sellers['date_first_sale']) / np.timedelta64(1, 'M'))
        sellers['total_revenue'] = sellers['olist_age'] * 80 + sellers['sales'] * 0.1
        ...
        # 2 Get matching table
        table = self.matching_table
        order_reviews = self.data['order_reviews']
        new_table = pd.merge(table[['seller_id', 'order_id']], order_reviews[['review_score', 'order_id']], how="inner", on="order_id")
        new_table['review_cost'] = new_table['review_score'].apply(func=cost_review)
        review_cost_seller = new_table.groupby('seller_id').sum()[['review_cost']]

        # 3 Création de new_sellers
        new_sellers = pd.merge(sellers, review_cost_seller, how="inner", on="seller_id")
        #new_sellers['it_cost'] = round(500_000 / sum(new_sellers['n_orders']**0.5) * new_sellers['n_orders']**0.5)
        #new_sellers['total_cost'] = new_sellers['it_cost'] + new_sellers['review_cost']

        # Get profit

        new_sellers['profit'] = new_sellers['total_revenue'] - new_sellers['review_cost']
